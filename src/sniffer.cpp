...
/*Sniffer::Sniffer(string dst_mac)
{
  ...
  //this->setAmqpParameter(host, port, username, password, svdata_exchange_name,key);
  //this->createProducerKafka(config);
  //this->amqpConnect();
...
Sniffer::Sniffer(string filepath, bool il, cppkafka::Configuration config)
{
  cout<<"Aquisition module has been started ...."<<endl;
  this->timeseg = -1;
...
void Sniffer::createProducerKafka(cppkafka::Configuration config)
{
  // Create the producer
...
void Sniffer::dataSender(SVData data) //Rotina de publicação dos dados para o RabbitMQ ou Kafka
{
  ...
  try
  {
    ...
    if(i<BATCHSIZE)
    {
      ...
      string message = message.dump();

      this->producer->produce(MessageBuilder("my_topic").partition(0).key(key).payload(message));
      this->producer->flush();
  ...
  {
    cout<<"Erro na conexão com o Kafka:"<< e.what()<<endl;
